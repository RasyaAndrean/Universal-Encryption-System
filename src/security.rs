...
impl RateLimiter {
    ...
    pub fn check_rate_limit(&mut self) -> Result<(), SecurityError> {
        ...
        self.attempts.retain(|&attempt| now.duration_since(attempt) < self.time_window);
...
pub fn validate_password_strength(password: &str) -> Result<(), SecurityError> {
    ...
    if uppercase < MIN_UPPERCASE {
        return Err(SecurityError::WeakPassword(
            format!("Password must contain at least {} uppercase letters", MIN_UPPERCASE)
        ));
    }
    
    if lowercase < MIN_LOWERCASE {
        return Err(SecurityError::WeakPassword(
            format!("Password must contain at least {} lowercase letters", MIN_LOWERCASE)
        ));
    }
    
    if digits < MIN_DIGITS {
        return Err(SecurityError::WeakPassword(
            format!("Password must contain at least {} digits", MIN_DIGITS)
        ));
    }
    
    if special < MIN_SPECIAL_CHARS {
        return Err(SecurityError::WeakPassword(
            format!("Password must contain at least {} special characters", MIN_SPECIAL_CHARS)
        ));
    }
