use aes_gcm::{Aes256Gcm, KeyInit, Nonce, aead::{Aead, Key}};
use rand::RngCore;
use sha2::{Sha256, Digest};
use zeroize::Zeroize;

use crate::crypto::key_derivation::{derive_key_from_password, derive_key_with_salt, DerivedKey, KeyDerivationError};
...
pub fn encrypt_data(
    ...
    if ciphertext.len() < TAG_LENGTH {
        return Err(EncryptionError::Encryption("Ciphertext too short".to_string()));
    }
    
    let (data, tag_bytes) = ciphertext.split_at(ciphertext.len() - TAG_LENGTH);
    let tag: [u8; 16] = tag_bytes.try_into()
